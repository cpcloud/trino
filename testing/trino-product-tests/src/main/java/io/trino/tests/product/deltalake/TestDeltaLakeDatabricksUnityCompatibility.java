...
public class TestDeltaLakeDatabricksUnityCompatibility
        extends ProductTest
{
    private String unityCatalogCatalogName;
    ...
    public void setUp()
    {
        unityCatalogCatalogName = requireNonNull(System.getenv("DATABRICKS_UNITY_CATALOG_NAME"), "Environment variable not set: DATABRICKS_UNITY_CATALOG_NAME");
        ...
        onDelta().executeQuery(
                format("CREATE SCHEMA %s.%s MANAGED LOCATION '%s'",
                        unityCatalogCatalogName, schemaName, schemaLocation));
    }

    @AfterMethodWithContext
    public void cleanUp()
    {
        onDelta().executeQuery(format("DROP SCHEMA IF EXISTS %s.%s CASCADE", unityCatalogCatalogName, schemaName));
    }
    ...
    public void testBasicTableReadWrite()
    {
        ...
        onDelta().executeQuery(
                format("CREATE TABLE %s.%s.%s (c1 int, c2 string) USING delta LOCATION '%s'",
                        unityCatalogCatalogName, schemaName, tableName, tableLocation));
        onDelta().executeQuery(
                format("INSERT INTO %s.%s.%s VALUES (1, 'one')",
                        unityCatalogCatalogName, schemaName, tableName));
        ...
        assertThat(onDelta().executeQuery(format("SELECT * FROM %s.%s.%s", unityCatalogCatalogName, schemaName, tableName)))
                .containsOnly(expectedRowsForInsert);
        ...
        onDelta().executeQuery(
                format("CREATE TABLE %s.%s.%s (c1 int, c2 string) using delta location '%s'",
                        unityCatalogCatalogName, schemaName, sourceTableName, tableLocation2));
        onDelta().executeQuery(
                format("INSERT INTO %s.%s.%s values (1, 'one'), (2, 'two'), (3, 'three')",
                        unityCatalogCatalogName, schemaName, sourceTableName));
    ...
    public void testTableColumns()
    {
        ...
        onDelta().executeQuery(format("CREATE TABLE %s.%s.%s (" +
                ...
                ") " +
                "USING DELTA " +
                "LOCATION '%s'", unityCatalogCatalogName, schemaName, tableName, tableLocation));
        assertThat(
